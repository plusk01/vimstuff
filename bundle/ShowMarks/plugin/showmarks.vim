...
if !exists('g:showmarks_ignore_buftype') | let g:showmarks_ignore_buftype = "nofile"  | endif
...
fun! s:ShowMarksSetup()
	...
	while n < s:maxmarks
		...
		let cmd= 'sign define ShowMark'.nm.' '.lhltext.' text='.text.' texthl='.s:ShowMarksDLink{nm}
		exe cmd
...
fun! s:ShowMarks()
	...
	if ( match( &buftype, g:showmarks_ignore_buftype ) > -1 )
		return
	endif
