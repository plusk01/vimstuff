...
function! s:GetValaIncludes()
    if exists('g:syntastic_vala_includes')
        if type(g:syntastic_vala_includes) == type('')
            return split(g:syntastic_vala_includes, '\s\+')
        elseif type(g:syntastic_vala_includes) == type([])
            return copy(g:syntastic_vala_includes)
        else
            echoerr 'g:syntastic_vala_includes must be either list or string: fallback to in file modules string'
        endif
    endif

    let vapi_line = search('^//\s*include:\s*','n')
    let vapi_str = getline(vapi_line)
    return split( substitute( vapi_str, '^//\s*include:\s*', '', 'g' ), '\s\+' )
endfunction
...
function! SyntaxCheckers_vala_valac_GetLocList() dict
	...
	let vala_include_args = join(s:GetValaIncludes(), ' ')

    let args  = '-C '
    let args .= vala_pkg_args . " "
    let args .= vala_vapi_args . " "
    let args .= vala_include_args 
